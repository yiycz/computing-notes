...
tags:
  ...
  - Theory
---
# General Terminology for Networking
...
A computer cannot communicate over a network on its own. The NIC acts as the communication interface, allowing it to:
...
The mask tells us that the first 24 bits are reserved for the network (192.168.1) and the last 8 bits are available for hosts. The IP 192.168.1.10 is therefore host number 10 on the 192.168.1.0 network.
...
|---|---|---|
|Connection|No dedicated connection|Dedicated connection established first|
|Data transmission|Split into packets|Continuous stream over one circuit|
|Route|Packets may take different routes|All data follows the same route|
|Bandwidth|Shared among users|Reserved for one connection|
|Efficiency|High|Lower (bandwidth can be wasted)|
|Reliability|Can reroute around failures|Circuit failure breaks communication|
|Delay|Variable|More predicatable|
|Data order|May arrive out of order|Arrives in order|
|Used by|Internet|Traditional telephone networks|
...
1. SYN (Synchronise): The client sends a packet with the SYN flag set to the server to establish a connection
    
2. SYN-ACK (Synchronise and Acknowledge): The server replies with another packet with both SYN and ACK flags on.
    
3. ACK (Acknowledge): The client replies with another packet with the ACK flag on to acknowledge that the server is listening
